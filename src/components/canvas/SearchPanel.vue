...
function focusItem(id: string, type: 'node' | 'edge') {
  if (type === 'node') {
    ...
    const maxX = Math.max(s.x + (graphStore.nodes[edge.sourceId]?.geometry.w ?? 0), t.x + (graphStore.nodes[edge.targetId]?.geometry.w ?? 0));
    const maxY = Math.max(s.y + (graphStore.nodes[edge.sourceId]?.geometry.h ?? 0), t.y + (graphStore.nodes[edge.targetId]?.geometry.h ?? 0));
    fitWorldBox({ x: minX, y: minY, w: maxX - minX, h: maxY - minY }, props.canvasWidth, props.canvasHeight, 80);
  }
}
...
<template>
  ...
  >
    ...
    <ul
      v-if="items.length > 0"
      class="max-h-[320px] overflow-y-auto text-sm"
    >
      <li
        v-for="item in items"
        :key="item.index"
