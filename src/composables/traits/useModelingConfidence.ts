...
export function useModelingConfidence(
  ...
  const openQuestionsCount = computed(
    () =>
      metadata.value.openQuestions.filter(
  ...
  const isFullyDocumented = computed(
    () =>
      maturity.value === ModelingMaturity.Verified || maturity.value === ModelingMaturity.Certified
  )
...
export function generateMaturitySvgStyle(
  maturity: ModelingMaturity
