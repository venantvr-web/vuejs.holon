...
describe('useFilterable', () => {
  ...
  beforeEach(async () => {
    ...
    await store.importNode(makeNode('domaine', null, { name: 'Domaine Ventes', archimateType: 'business-function' }));
    await store.importNode(makeNode('appli', 'domaine', { name: 'CRM', archimateType: 'application-component' }));
    await store.importNode(makeNode('serveur', 'appli', { name: 'Serveur CRM', archimateType: 'technology-node' }));
    await store.importNode(makeNode('autre', null, { name: 'Référentiel RH', archimateType: 'application-component' }));
  });
