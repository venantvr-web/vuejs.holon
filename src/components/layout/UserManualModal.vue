...
function parseMarkdown(md: string): string {
  ...
  for (const rawLine of lines) {
    ...
    // Headings
    const h4 = line.match(/^#### (.*)$/);
    if (h4) { flushPara(); closeList(); out.push(`<h4>${h4[1]}</h4>`); continue; }
    const h3 = line.match(/^### (.*)$/);
    if (h3) { flushPara(); closeList(); out.push(`<h3>${h3[1]}</h3>`); continue; }
    const h2 = line.match(/^## (.*)$/);
    if (h2) { flushPara(); closeList(); out.push(`<h2>${h2[1]}</h2>`); continue; }
...
function next() { if (canNext.value) index.value++; }
function prev() { if (canPrev.value) index.value--; }
function jumpTo(i: number) { index.value = Math.max(0, Math.min(i, total.value - 1)); }
