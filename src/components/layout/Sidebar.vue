...
<template>
  <aside class="w-64 bg-gray-100 border-r flex flex-col overflow-hidden">
    <!-- Bibliothèque -->
    <div class="flex-shrink-0 border-b">
      <button
        class="w-full flex items-center justify-between px-3 py-2 hover:bg-gray-200"
        @click="libraryOpen = !libraryOpen"
      >
        <span class="text-sm font-bold">{{ libraryOpen ? '▾' : '▸' }} {{ t('sidebar.library') }}</span>
        <span class="text-xs text-gray-500">{{ libraryStore.items.length }}</span>
      </button>
      <div v-if="libraryOpen" class="p-2 max-h-[40vh] overflow-y-auto">
        <ul>
          <li
            ...
            :key="item.id"
            class="group flex items-center justify-between p-2 border rounded bg-white cursor-grab mb-2 hover:border-blue-400"
            draggable="true"
            @dragstart="handleDragStart($event, item)"
          >
            <span class="truncate flex-1 text-sm">
              <span v-if="item.isBuiltIn" class="text-gray-400 text-xs mr-1">●</span>
              {{ item.name }}
            </span>
            <button
              v-if="!item.isBuiltIn"
    ...
    <div class="flex-1 flex flex-col overflow-hidden">
      <button
        class="flex-shrink-0 w-full flex items-center justify-between px-3 py-2 hover:bg-gray-200"
